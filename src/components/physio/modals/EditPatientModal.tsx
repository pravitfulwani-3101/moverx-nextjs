...
export function EditPatientModal({ patient, form, onChange, onSubmit, onDelete, onClose }: EditPatientModalProps) {
  ...
  return (
    ...
    >
      <div
        className="modal-sheet-inner modal-sheet-tall w-[92%] max-w-[520px] max-h-[90vh] overflow-y-auto rounded-[20px] p-7"
        style={{ background: "#151b28", border: "1px solid rgba(255,255,255,0.08)" }}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="text-[18px] font-black mb-1" style={{ color: "#f0f6fc" }}>✏️ Edit Member</div>
        ...
        <div className="flex flex-col gap-3.5">
          ...
          <div className="flex gap-3">
            ...
            <div className="flex-1">
              ...
              <select value={form.dominantHand} onChange={(e) => set("dominantHand", e.target.value)} style={selectStyle}>
                <option value="">Select</option>
                {DOMINANCE_OPTIONS.map((d) => <option key={d} value={d}>{d}</option>)}
              </select>
            </div>
          </div>
          ...
          <div className="flex gap-3">
            <div className="flex-1">
              ...
              <select value={form.lifestyle} onChange={(e) => set("lifestyle", e.target.value)} style={selectStyle}>
                <option value="">Select</option>
          ...
          <div>
            ...
            <input value={form.complaintP1} onChange={(e) => set("complaintP1", e.target.value)} placeholder="Primary complaint..." style={inputStyle} />
          </div>
          <div>
            {fieldLabel("P2 — Secondary Complaint")}
            <input value={form.complaintP2} onChange={(e) => set("complaintP2", e.target.value)} placeholder="Secondary complaint..." style={inputStyle} />
          </div>
          <div>
            {fieldLabel("P3 — Tertiary Complaint")}
            <input value={form.complaintP3} onChange={(e) => set("complaintP3", e.target.value)} placeholder="Tertiary complaint..." style={inputStyle} />
          </div>
          ...
          >
            {showClinicalNotes ? "▾ Hide Clinical Notes" : "▸ Edit Clinical Notes"}
          </button>
                ...
                <div className="flex flex-col gap-4 mt-2">
                  {ASSESSMENT_TYPES.map(({ key, label }) => (
                    ...
                    >
                      ...
                      >
                        <span>{label}</span>
                        <span style={{ color: "#4a5568" }}>{activeAssessment === key ? "▾" : "▸"}</span>
                      </button>
                      ...
                      {activeAssessment === key && (
                        <div className="p-3">
                            ...
                            {VIEWS.map((view) => {
                              ...
                              return (
                                <div key={view} className="flex flex-col items-center">
                                  <button
                                    type="button"
                                    onClick={() => setActiveView(isActive ? null : { type: key, view })}
                                    ...
                                    style={{
                                      ...
                                      background: isActive ? "rgba(168,85,247,0.1)" : hasNotes ? "rgba(34,197,94,0.06)" : "rgba(255,255,255,0.02)",
                                    }}
                                  >
                                    <StickFigure view={view} size={60} color={isActive ? "#a855f7" : hasNotes ? "#22c55e" : "#4a5568"} />
                                  </button>
                                  {hasNotes && <div className="w-1.5 h-1.5 rounded-full mt-1" style={{ background: "#22c55e" }} />}
                                </div>
                              );
                            })}
                          </div>
                          {activeView?.type === key && (
                            <div className="mt-3">
                              {fieldLabel(`${label} — ${activeView.view.charAt(0).toUpperCase() + activeView.view.slice(1)} View Notes`)}
                              <textarea
